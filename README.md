...
<head>
    ...
    <!-- Application Structure Plan: La estructura de la SPA es una narrativa de una sola página que sigue el flujo lógico del reporte: 1) Encabezado y Crisis, 2) Sección de "Acción Inmediata" (Punto 1.a) con tarjetas de acción y el gráfico de Dona, 3) Sección "Hoja de Ruta de la Reforma" (Punto 1.b) usando una línea de tiempo de HTML/Tailwind y el gráfico de Barras, y 4) Sección "Vigilancia Permanente" (Punto 1.c) con tarjetas para la "Doble Vigilancia" y el gráfico de Líneas. Esta estructura temática (Problema -> Solución -> Prevención) fue elegida por ser la más intuitiva para que un usuario comprenda el plan de acción completo de Gobierno Corporativo. -->
    <!-- Visualization & Content Choices: 
        1. Datos (1.a - Focos de Fraude): Meta -> Informar (Composición). Viz -> Gráfico de Dona (Chart.js). Justificación -> Muestra claramente la proporción del principal foco de auditoría ("Gastos Operativos"). Sin SVG.
        2. Datos (1.b - Reforma del Consejo): Meta -> Comparar (Categorías Antes/Después). Viz -> Gráfico de Barras Agrupado (Chart.js). Justificación -> Compara directamente la cuenta de tipos de consejeros (Ejecutivos vs. Independientes) para mostrar el cambio. Sin SVG.
        3. Datos (1.c - Impacto de Vigilancia): Meta -> Mostrar Cambio/Relación (Tendencia en el tiempo). Viz -> Gráfico de Líneas Combinado (Chart.js) con 2 ejes Y. Justificación -> Demuestra la correlación inversa entre el aumento de reportes éticos y la disminución de pérdidas por fraude. Sin SVG.
        4. Datos (1.b - Fases del Plan): Meta -> Organizar (Proceso). Viz -> Línea de Tiempo HTML/Tailwind. Justificación -> Muestra la secuencia de los pasos de la reforma sin usar SVG o Mermaid JS, cumpliendo los requisitos.
    -->
    ...
    <style>
        ...
        /* Estilos para la Línea de Tiempo (Sin SVG/Mermaid) */
        .timeline-line {
            position: absolute;
            left: 50%;
            ...
            bottom: 20px;
            width: 4px;
            background-color: #A9D0DE; /* Light Blue */
            border-radius: 2px;
        }
        .timeline-item {
            position: relative;
        ...
        .timeline-content {
            ...
            border-top: 4px solid #4771B2; /* Medium Blue accent */
        }
        ...
        .timeline-item::after { /* Círculo en la línea */
            content: '';
            position: absolute;
            width: 20px;
            height: 20px;
            background-color: white;
            border: 4px solid #00429D; /* Dark Blue */
            border-radius: 50%;
            top: 50%;
            left: 50%;
        ...
        .timeline-item::before { /* Limpiador de float */
            content: "";
            display: table;
            clear: both;
        }
...
<body class="bg-gray-100">
    ...
    <script>
        // Función de utilidad para manejar etiquetas de múltiples líneas en Tooltips de Chart.js
        ...
        // Configuración común de Tooltip requerida
        ...
        // --- Gráfico 1: Focos de Auditoría (Dona) ---
        const auditCtx = document.getElementById('auditChart').getContext('2d');
        new Chart(auditCtx, {
            type: 'doughnut',
            data: {
                // Aplicando lógica de envoltura de 16 caracteres
        ...
        // --- Gráfico 2: Reforma del Consejo (Barras Agrupadas) ---
        ...
        // --- Gráfico 3: Impacto de la Vigilancia (Líneas Combinadas) ---
